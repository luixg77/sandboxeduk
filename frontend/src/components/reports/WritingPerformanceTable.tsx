...
interface WritingPerformanceTableProps {
...
export const WritingPerformanceTable: React.FC<WritingPerformanceTableProps> = ({ students, gabarito }) => {
  ...
  const filteredStudents = useMemo(() => {
    ...
    return [...result].sort((a, b) => {
      switch (sortOption) {
        ...
        case "writingFinalScore_desc":
          return b.writingFinalScore - a.writingFinalScore;
        case "writingFinalScore_asc":
          return a.writingFinalScore - b.writingFinalScore;
        default:
          return 0;
      }
    });
  ...
  const columns = [
    ...
    {
      header: "Gênero e Tema (4)",
      accessor: (row: StudentPerformance) => <span className="font-medium text-gray-600">{row.writingAdequation}</span>,
    },
    {
      header: "Coerência (4)",
      accessor: (row: StudentPerformance) => <span className="font-medium text-gray-600">{row.writingCoherence}</span>,
    },
    {
      header: "Estrutura (4)",
      accessor: (row: StudentPerformance) => <span className="font-medium text-gray-600">{row.writingStructure}</span>,
    },
    {
      header: "Ortografia (4)",
      accessor: (row: StudentPerformance) => <span className="font-medium text-gray-600">{row.writingOrthography}</span>,
    },
    {
      header: "Nota Final (100)",
      accessor: (row: StudentPerformance) => <span className="font-bold text-gray-800">{row.writingFinalScore.toFixed(2)}</span>,
    },
    {
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-6">
      <div className="mb-6">
        <h3 className="text-lg font-bold text-gray-800 mb-4">Desempenho Individual (Produção Textual)</h3>
            ...
            <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-lg pl-3 pr-2 py-1.5 focus-within:ring-2 focus-within:ring-kodar-500 focus-within:border-kodar-500 transition-all hover:bg-gray-100/50">
              ...
              <div className="relative flex-1">
                ...
                >
                  ...
                  <option value="writingFinalScore_desc">Nota Final - Maior Nota</option>
                  <option value="writingFinalScore_asc">Nota Final - Menor Nota</option>
                </select>
